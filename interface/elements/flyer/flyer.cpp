...
namespace CPGL {
    void Flyer::draw()
    {
        ...
        Affine3f scale, mybase; scale.setIdentity();
        scale.linear().diagonal().setConstant(config["scale"].as<float>(1.0));

        // Send in additional params
        glUniformMatrix4fv(glGetUniformLocation(program, "projectionMatrix"), 1, GL_FALSE, get_projection());
        glUniformMatrix4fv(glGetUniformLocation(program, "baseMatrix"), 1, GL_FALSE, ((get_base())*scale).data());
